  ...
  (let ((tool-name (alist-get 'name params))
    ...
    (condition-case err
        (let* ((result
          ...
          ;; Deferred tool: it will send its response asynchronously.
          ;; Exit cleanly; the thrown value is informational only (like 'success below).
...
TEXT is the plain-text result string returned to the LLM.  This function
always produces a single text content item; it does not support multiple
content items or non-text types.  For those cases, build the response
hash table manually and call `mcp-server-transport-send-raw' directly.
Call this from an async tool handler after the user has provided input."
  ;; Use direct hash table approach to avoid alist conversion issues
  ;; with json-serialize (same pattern as --handle-tools-call).
  (let ((response-hash (make-hash-table :test 'equal))
        (result-hash   (make-hash-table :test 'equal))
